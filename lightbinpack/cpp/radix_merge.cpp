...
inline bool has_min_prefix_match(const std::vector<int>& list1, const std::vector<int>& list2, int min_prefix_match) {
    int match_length = 0;
    int min_size = std::min(static_cast<int>(list1.size()), static_cast<int>(list2.size()));
...
inline int max_shared_prefix(const std::vector<int>& list1, const std::vector<int>& list2) {
    int match_length = 0;
    int min_size = std::min(static_cast<int>(list1.size()), static_cast<int>(list2.size()));
    
    while (match_length < min_size && list1[match_length] == list2[match_length]) {
        match_length++;
    }
...
public:
    ...
    const std::vector<int>& get_flattened() const {
        return flattened;
    }

    const std::vector<uint64_t>& get_indices() const {
        return indices;
    }
...
radix_merge(
    ...
    if (allow_cross_group_merge) {
        ...
        for (size_t i = 1; i < input_data.size(); ++i) {
            ...
            for (const auto& existing_list : current_group) {
                for (const auto& next_list : input_data[i]) {
                    if (has_min_prefix_match(existing_list, next_list, min_prefix_match)) {
                        can_merge = true;
                        break;
                    }
                }
            ...
            if (can_merge && (current_count + static_cast<int>(input_data[i].size()) <= max_count)) {
                std::vector<std::vector<int>> temp_group = current_group;
                temp_group.insert(temp_group.end(), input_data[i].begin(), input_data[i].end());
                
                size_t original_size = flattener.get_flattened().size();
                for (size_t list_idx = 0; list_idx < input_data[i].size(); ++list_idx) {
                    flattener.insert_list(input_data[i][list_idx], current_group.size() + list_idx);
                }
                flattener.update_flattened();
        ...
        for (const auto& group : input_data) {
            ...
            if (group.size() == 1) {
                ...
                int merged_total_length = static_cast<int>(merged_flattened.size());
    ...
    return std::make_tuple(
        std::move(result), 
        std::move(total_lengths),
        std::move(flattened_lists),
        std::move(index_lists)
    );
}

PYBIND11_MODULE(radix_merge, m) {
    m.doc() = "Radix merge implementation for integer lists with shared prefix optimization and bitmask indices";
    m.def("radix_merge", &radix_merge,
          "Merge lists based on prefix matching and length/count constraints, considering shared prefixes and returning bitmask indices",
          py::arg("input_data"),
          py::arg("min_prefix_match") = 0,
          py::arg("max_length") = 16384,
          py::arg("max_count") = 32,
