...
class Bin
{
  public:
    ...
    Bin(double space, size_t index) : remaining_space(space), bin_index(index)
    {
    }
};

std::vector<std::vector<int>> bfd(const std::vector<double> &lengths, double batch_max_length)
{
    ...
    std::sort(length_pairs.begin(), length_pairs.end(), std::greater<std::pair<double, int>>());
...
{
    m.doc() = "BFD (Best Fit Decreasing) algorithm optimized implementation in C++";
