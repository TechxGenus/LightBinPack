...
public:
    ...
    int get_max_remaining() const {
        ...
        for (auto space : remaining_space) {
            if (space > max_val) max_val = space;
        }
        return max_val;
    }
...
std::vector<std::vector<std::vector<int>>> ohgbfd(
    ...
    if (strategy == 0) {
        ...
        std::vector<std::vector<size_t>> capacity_to_bins(max_batch_length + 1);
        std::vector<size_t> bins_remaining;
        bins_remaining.reserve(lengths.size() / 2);        

        bins_remaining.push_back(max_batch_length);
        capacity_to_bins[max_batch_length].push_back(0);
        seg_tree.update(max_batch_length, max_batch_length);
        ...
        for (int size = item_max_length; size >= 1; --size) {
            for (int orig_idx : count[size]) {
                ...
                if (best_capacity != -1) {
                    ...
                    bins_remaining.push_back(max_batch_length - size);
                    bins_items.emplace_back();
                    bins_items.back().push_back(orig_idx);

                    int new_capacity = max_batch_length - size;
        ...
        std::vector<std::vector<std::vector<int>>> result;
        for (size_t i = 0; i < bins_items.size(); i += batch_max_lengths.size()) {
            std::vector<std::vector<int>> group;
            for (size_t j = 0; j < batch_max_lengths.size(); j++) {
