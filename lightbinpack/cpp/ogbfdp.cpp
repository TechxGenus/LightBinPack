...
std::vector<std::vector<std::vector<int>>> ogbfd_worker(
...
std::vector<std::vector<std::vector<int>>> ogbfdp(
    const std::vector<int>& lengths,
    int batch_max_length,
    int bins_per_group = 1,
    ...
    for (int i = 0; i < num_threads; ++i) {
        parallel_results[i] = ogbfd_worker(lengths, groups[i], batch_max_length, bins_per_group, item_max_length, strategy);
    }
    ...
    if (!repack_items.empty()) {
        ...
        auto repacked = ogbfd_worker(lengths, flattened_repack, batch_max_length, bins_per_group, item_max_length, strategy);
...
PYBIND11_MODULE(ogbfdp, m) {
    m.doc() = "Parallel Optimized Grouped BFD (Best Fit Decreasing) algorithm implementation";
