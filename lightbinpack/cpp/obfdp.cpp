...
std::vector<std::vector<int>> obfd_worker(const std::vector<int> &lengths, const std::vector<int> &indices,
                                          int batch_max_length, int item_max_length)
{
    if (indices.empty())
    {
        return {};
...
std::vector<std::vector<int>> obfdp(const std::vector<int> &lengths, int batch_max_length, int item_max_length = -1)
{
...
PYBIND11_MODULE(obfdp, m)
{
    m.doc() = "Parallel Optimized BFD (Best Fit Decreasing) algorithm "
              "implementation";
    m.def("obfdp", &obfdp, "Parallel Optimized BFD algorithm", py::arg("lengths"), py::arg("batch_max_length"),
